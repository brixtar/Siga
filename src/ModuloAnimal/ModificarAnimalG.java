 ...
 * @author BayBay
 */
public class ModificarAnimalG extends javax.swing.JInternalFrame {

    /**
     * Creates new form ModificarAnimalP
     */
    public ModificarAnimalG() {
        initComponents();
    ...
    private void cargarTabla(){
        ...
        String colName[] = {"apellido","nombre","dni_duenio","animal_grandes.nombre","especie","raza","peso","nacimiento","sexo","color","vivo","cod_animal"};        
//nombre de columnas, tabla, instruccion sql
        dtAnimal = animal.GetTabla(colName, "animal_grandes, duenio,lista_especieg,lista_razag", "SELECT  `duenio`.`nombre`, `duenio`.`apellido`,`lista_especieg`.`especie`, `lista_razag`.`raza`,`animal_grandes`.*FROM animal_grandes, duenio,lista_especieg,lista_razag WHERE (`duenio`.`dni_duenio` =`animal_grandes`.`dni_duenio`) AND (`animal_grandes`.`cod_especie` =`lista_especieg`.`cod_especie`) AND (`animal_grandes`.`cod_raza` =`lista_razag`.`cod_raza`)");
...
//nombre de columnas, tabla, instruccion sql
    ...
    private void initComponents() {
        ...
        setClosable(true);
        setTitle("Modificar Animal Grandes");
        ...
        jPanel2.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "BUSCAR ANIMAL GRANDES", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 1, 11))); // NOI18N
    ...
    private void jbActualizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbActualizarActionPerformed
        ...
        //array de datos , instruccion sql
        animal.UpdateRegistro(datos, "UPDATE `animal_grandes`SET`nombre`=?,`cod_especie`=?,`cod_raza`=?,`peso`=?,`nacimiento`=?,`sexo`=?,`color`=?,`dni_duenio`=?,`vivo`=? WHERE `cod_animal`=?");
        cargarTabla();
    ...
    private void jbBuscarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbBuscarActionPerformed
        ...
        String[] columNames = {"Apellido","Nombre","DNI","Nombre","Especie","Raza","Peso","Fecha","Sexo","Color","Vivo","Codigo Animal"};          
        String colName[] = {"apellido","nombre","dni_duenio","animal_grandes.nombre","especie","raza","peso","nacimiento","sexo","color","vivo","cod_animal"};         //nombre de columnas, tabla, instruccion sql
        //nombre de columnas, tabla, instruccion sql
        if(jrbDni.isSelected()==true){
            dtAnimal = animal.GetTabla(colName, "animal_grandes, duenio,lista_especieg,lista_razag", "SELECT  `duenio`.`nombre`, `duenio`.`apellido`,`lista_especieg`.`especie`, `lista_razag`.`raza`,`animal_grandes`.*FROM animal_grandes, duenio,lista_especieg,lista_razag WHERE `duenio`.`dni_duenio` =`animal_grandes`.`dni_duenio` AND `animal_grandes`.`cod_especie` =`lista_especieg`.`cod_especie` AND `animal_grandes`.`cod_raza` =`lista_razag`.`cod_raza` AND `duenio`.`dni_duenio`="+texto+" ");
        }
        if(jrbMascota.isSelected()==true){

            dtAnimal = animal.GetTabla(colName, "animal_grandes, duenio,lista_especieg,lista_razag", "SELECT  `duenio`.`nombre`, `duenio`.`apellido`,`lista_especieg`.`especie`, `lista_razag`.`raza`,`animal_grandes`.*FROM animal_grandes, duenio,lista_especieg,lista_razag WHERE (`duenio`.`dni_duenio` =`animal_grandes`.`dni_duenio`) AND (`animal_grandes`.`cod_especie` =`lista_especieg`.`cod_especie`) AND (`animal_grandes`.`cod_raza` =`lista_razag`.`cod_raza`) AND (`animal_grandes`.`nombre`= \"" + texto + "\") ");
        }
        // se colocan los datos en la tabla
        DefaultTableModel datos = new DefaultTableModel(dtAnimal,columNames);
        jtDuenio.setModel(datos);
